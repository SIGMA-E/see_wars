# import os # не получилось заставить работать cls в PyCharm
from random import randrange
from random import choice


# разбиваем игру на 3 части: поле для игры, используемые ходы, и вес ячейки (коэффициент вероятности для ИИ)
class FieldPart(object):
    main = 'map'
    radar = 'radar'
    weight = 'weight'  # для повышения IQ нашего ИИ


# добавим чуть-чуть цветов в нашу черно-белую обыденность. (их работа проверялась на Win 10)
class Color:
    grey = '\033[0;37m'
    reset = '\033[0m'
    blue = '\033[0;34m'
...
# функция которая окрашивает объекты
def set_color(text, color):
    return color + text + Color.reset


# класс "Cell". Здесь мы задаем визуальное отображение клеток и их цвет.
class Cell(object):
    empty_cell = set_color('-', Color.grey)
...
# поле с весом клеток. чтоб в дальнейшем избежать беспорядочных ходов ИИ
class Field(object):
    ...
    def draw_field(self, element):
        ...
        if element == FieldPart.weight:
            ...
            # Создаем игровое поле для нашей игры
    ...
    # помечаем все клетки вокруг уничтоженного корабля - сыгранными
    # и вывдим сам уничтоженный корабль
    ...
    def recalculate_weight_map(self, available_ships):
        ...
        # и если он помещается прибавляем клетке коэф 1.
            ...
            for x in range(self.size):
                for y in range(self.size):
                    if self.radar[x][y] in (Cell.destroyed_ship, Cell.damaged_ship, Cell.miss_cell) \
                        ...
                        self.weight[x][y] = 0
                        continue
                    # вот здесь проверяем помещается ли корабль
...
class Game(object):
        ...
        for ship_size in Game.ships_rules:
            ...
            while True:
                ...
                if player.auto_ship_setup is not True:
                    ...
                    player.message.append(f'Куда поставить {ship_size} корабль: ')
    ...
    def draw(self):
        if not self.current_player.is_ai:
            ...
            # расчет веса соседних клеток, начиная из центра:
    ...
    @staticmethod
    def clear_screen():  # нужно дорабатывать
        pass
...
if __name__ == '__main__':
    print('----------------------------------------------------------')
    print('Добро пожаловать в игру "Морской бой ver.1.0" бета-версия!')
    print('----------------------------------------------------------')
    print("""
«Морской бой» — игра для двух участников, в данном случае Вы будете играть против Искусственного
интеллекта, в которой игроки по очереди называют координаты на неизвестной им карте соперника. 
Если у соперника по этим координатам имеется корабль (координаты заняты), то корабль или его часть
«топится», а попавший получает право сделать ещё один ход. Цель игрока — первым потопить все корабли
противника.
    """)
    print('    ПРАВИЛА ИГРЫ:\n'
          '* Игровое поле - 6 х 6 клеток со случайным расположением кораблей как у "ИИ" так и у "Игрока"\n'
          '* По вертикали расположены координаты с латинскими буквами A...F\n'
          '* По горизонтали расположены координаты с цифрами 1...6\n'
          '* Количество кораблей ограничено: трехпалубный крейсер - 1 шт., двухпалубный эсминец - 2 шт.\n'
          '  однопалубный катер - 4 шт.\n'
          '* В процессе игры нужно прописывать координаты выстрелов в формате буква и цифра, напрмер: A2 или a2\n'
          '* Промахи отображаются жирной точкой желтого цвета\n'
          '* Попадания окрашивают корабль красным цветом\n'
          '* В этой версии ИИ будет искать более вероятные точки для выстрелов, так что победить его будет непросто!!!')
    print()
    input("Нажмите Enter чтобы продолжить.")
    name_1 = input('Введите Ваше имя или никнейм: ')
    print('Игровые поля загружены. Корабли расставлены. Пусть победит сильнейший!')
    print()

    # здесь делаем список из двух игроков и задаем им основные параметры
    players = []
    players.append(Player(name=name_1, is_ai=False, auto_ship=True, skill=1))
    players.append(Player(name='T-1000M', is_ai=True, auto_ship=True, skill=1))
    ...
    while True:
        ...
        if game.status == 'in game':
            ...
            # Game.clear_screen()  # пока работает некорректно - будем разбираться
            game.current_player.message.append("Открыть огонь по координатам \N{Neutral Face}: ")
            game.draw()
            ...
            if shot_result == 'miss':
                game.next_player.message.append('Игрок {}, промахнулся! \N{Face with Pleading Eyes}'.format(game.current_player.name))
                ...
                game.current_player.message.append('Отличный выстрел, продолжайте!\N{Grinning Face}')
                game.next_player.message.append('Наш корабль попал под обстрел!\N{Astonished Face}')
                continue
            elif shot_result == 'kill':
                game.current_player.message.append('Корабль противника уничтожен!\N{Grinning Face}')
                game.next_player.message.append('Плохие новости, наш корабль был уничтожен \N{Tired Face}')
                continue
        ...
        if game.status == 'game over':
            ...
            print('Игрок {} выиграл матч! Поздравления!\N{Clapping Hands Sign}'.format(game.current_player.name))
            break
